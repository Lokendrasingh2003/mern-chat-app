...
const SingleChat: React.FC<ChatBoxProps> = ({ fetchAgain, setFetchAgain }) => {
	...
	useEffect(() => {
		...
		const handleMessageReceived = (newMessageReceived: MessageProps) => {
		...
		socket.on<SocketNames>('messageReceived', handleMessageReceived);
		return () => {
			if (socket) {
				socket.off('messageReceived', handleMessageReceived);
			}
		};
	}, [selectedChat, setFetchAgain]);
	...
	const fetchMessages = async () => {
		...
		try {
			...
			if (socket) {
				socket.emit<SocketNames>('joinChat', selectedChat);
			}
		} catch (error) {
			console.error(error);
			toast({
